...
get_type <- . %>%  mutate(vtype=ifelse(var %in% c("incidence","report"),
                                "inc","prev"))
...
gg4 <- (ggplot(forecast1,aes(date,value,colour=var))
    ...
    + geom_text(data=newtest_lab, aes(label=lab))
    + labs(y="")
...
plotfun <- function(data,
    ...
    force(limspace)
    print(gg4 %+% data
          + geom_dl(method=list(dl.trans(x=x+1),cex=1,"last.points"),
                    aes(label=var))
...
plotfun(mk_fc(new_end="15-05-2020"),limspace=20)
```

### forecast much farther ahead

```{r forecast_longlong,fig.width=6,fig.height=7, warning=FALSE}
plotfun(mk_fc(new_end="01-10-2020"),limspace=50)
```
