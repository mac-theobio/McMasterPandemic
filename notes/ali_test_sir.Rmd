...
# checking for t=0, if simple R_0 for SIR can be recovered?
# Case #1: t=0
...
Note that once the only untested people are $S_u$, the FoI $\Lambda=0$, $F_s=t N_0/S_u$. Thus,
in the eq (1) of the model $d S_u/dt = - t N_0 + \omega S_n$ which is a linear rate of leaving the $S_u$ compartment. That means that the testing rate will be no longer defendant on $S_u$. 
...
```{r show_contours, width=16,height=16
p1 <- ggplot(df2,aes(x=Fi,y=omega,z=R0))+ theme_bw() +
