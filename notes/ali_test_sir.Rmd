...
sir.model <- function(time,state,params){
  ...
  unpack(as.list(state,params))
  
  # scaling the weights
  F1 <- t*(W_s*N0)/(W_s*S_u+W_i*I_u+W_r*R_u)
  F2 <- t*(W_i*N0)/(W_s*S_u+W_i*I_u+W_r*R_u)
  F3 <- t*(W_r*N0)/(W_s*S_u+W_i*I_u+W_r*R_u)
  ...
  dxdt <- list(c(dS_u.dt,dS_n.dt,dI_u.dt,dI_n.dt,dI_p.dt,dI_t.dt,dR_u.dt,dR_n.dt,dR_p.dt,dR_t.dt,dN.dt,dP.dt))  
    })
...
params <- c(N0=100, beta=1,gamma=1/3, omega=0.25, t=3, 
