...
gamma <- 1
eta1  <- seq(0,1, length.out=11)
P_I   <- 0.5
t     <- seq(0,0.1, length.out=11)
omega <- seq(0, 1, length.out=11)
eta2  <- seq(0, 0.1, length.out=11)

df1 <- expand.grid(beta=beta,gamma=gamma,eta1=eta1,t=t,omega=omega,eta2=eta2,P_I=P_I)
...
## df2_long <- tidyr::pivot_longer(df2,-c(FI,omega,eta2,t))

p1<- ggplot(df2,aes(x=FI,y=omega,z=R0))
p1 + geom_contour(breaks=1) + facet_grid(eta1~eta2, labeller=label_both)
```
