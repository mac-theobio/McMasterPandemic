...
\begin{align}
\label{mod:1}
1)\  d S_u/dt &= -\beta \frac{(I_u+\eta_w I_n+\eta_w I_p+ \eta_t I_t)}{N_0} S_u - t (1-P_S) F(W_s) S_u +\omega S_n \\
2)\ d S_n/dt &= -\beta \frac{(I_u+\eta_w I_n+\eta_w I_p+ \eta_t I_t)}{N_0} S_n + t (1-P_S) F(W_S) S_u - \omega S_n   \\
3)\ d I_u/dt &= \beta \frac{(I_u+\eta_w I_n+\eta_w I_p+ \eta_t I_t)}{N_0} S_u -t F(W_I) I_u + \omega I_n  - \gamma I_u  \\
4)\ d I_n/dt &= \beta \frac{(I_u+\eta_w I_n+\eta_w I_p+ \eta_t I_t)}{N_0} S_n +t (1-P_I) F(W_I) I_u - \omega I_n -\gamma I_n \\
5)\ d I_p/dt &= t P_I F(W_I) I_u - \omega I_p -\gamma I_p \\
...
$\eta$ is the isolation parameter with the assumption of $\eta_t<\eta_w$.
