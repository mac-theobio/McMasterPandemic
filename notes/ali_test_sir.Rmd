...
For convenience, let $F_S=t*F(W_S), F_I=t*F(W_I)$ and $F_R=t*F(W_R)$ hereafter. 
...
\label{exp:dfe}
- The Disease-Free Equilibrium, $E_1$: $S_u= \frac{\omega}{\omega+t (1-P_S) F(W_S)} N_0, S_n= N_0-S_u$, and $I_j=R_j=0 \ \forall j$.
- (Not sure what is the right term for this one?), $E_2$.

### Final State, E_2

Let $S=S_u + S_n$, $I=I_u + I_n + I_p + I_t$ and $R=R_u + R_n + R_p + R_t$.
We know $S+I+R=N_0$.

$dI/ds=\frac{\gamma}{\beta S} N_0 -1$, thus $I = \frac{\gamma N_0}{\beta} \log(S) - S +C$, where $C= N_0(1-\frac{\gamma}{\beta} \ln(N_0))$.

$dS/dR = \frac{-\beta}{\gamma N_0} S$, thus $S(R) = S(0) \exp(\frac{-\beta}{\gamma N_0}R)$

$dR/dt=\gamma I = \gamma (N_0-S-R)$.

as $t \rightarrow \infty, I \rightarrow 0,  dR/dt \rightarrow 0 $. Thus, $R_{\infty} = N_0 ( 1-\exp(\frac{-\beta}{\gamma N_0}R_{\infty}))$ which has a unique solution given that $\beta/\gamma N_0 >0$.

Thus, as $t \rightarrow \infty$, $S \rightarrow S_{\infty}, I \rightarrow 0$ and $R \rightarrow R_{\infty}$. 
...
\beta \eta_1 F_I (1-P_I) & \beta \eta_1 (\gamma +F_I)
\end {array} \right],
\\ 
G_{22} =
\left[ \begin {array}{cc}
...
$b=-(\beta (\omega+\gamma) + \beta \eta_1 (\gamma + F_I))$ and
$c= \beta^2 \eta_1$. Thus,
$$
...
$$\lambda_3 =\frac{\beta \eta_1}{\omega+\gamma} \\
\lambda_4 =\frac{\beta \eta_2}{\gamma}$$

Notes:

- $\lambda_2 \leq \lambda_1$, since $b*c>0$.
- $\lambda_4 \geq \lambda_3 \iff \omega/\gamma+1 \geq \eta_1/\eta_2$.

**Proposition 1**
If $\eta_1*(\frac{F_I}{\gamma} +1)*(\frac{\omega}{\omega+\gamma}+1)\leq 1$ (\label{cond:1}), then $\cal{R_0}=\lambda_1$.
Brief proof; show that $\lambda_3\leq\lambda_1$ and $\lambda_4\leq\lambda_1$. Also the assumption is that $\eta_2\leq\eta_1$.
...
(1-P_I) F_I& \beta \eta_1-(\omega+\gamma)
\end {array} \right]$, 

$J_{33} = \left[ \begin {array}{cc} 
\beta \eta_1-(\omega+\gamma)& 0 \\
\omega& \beta \eta_2-\gamma
\end {array} \right]$, 
...
# just a test
F_I(state=state_init,params=subset(params,!names(params) %in% c("t")),t=0)
...
eta1 <- 1
P_I <- 0.5
t <- seq(0,0.05,by=0.005)
FI<-F_I(state=state_init,params=subset(params,!names(params) %in% c("t")),t=t)
omega <- seq(0,0.5,by=0.05)
eta2 <- seq(0,0.05,by=0.005)


df1<- expand.grid(beta=beta,gamma=gamma,eta1=eta1,t=t,FI=FI,omega=omega,eta2=eta2,P_I=P_I)

# Define the eigenvalues of next generation matrix G:
lam1<- function(input){
  unpack(input)
  a<- (omega+gamma)*gamma+(gamma+omega*P_I)*FI
    b<--(beta*(omega+gamma) + beta*eta1*(gamma + FI))
    c<-beta^2*eta1
...
lam3 <- function(input){
  unpack(input)
  return((beta*eta1)/(omega+gamma))
}

lam4 <- function(input){
  unpack(input)
  return((beta*eta2)/(gamma))
}
...
df2 <- data.frame(df1,"lambda1"=lambda1,"lambda3"=lambda3,"lambda4"=lambda4) #,"R0"=R0

df2[, "R0"] <- apply(df2[, names(df2)%in%c("lambda1","lambda3","lambda4")], 1, max)
which(df2[]<0)

df2_long <- tidyr::pivot_longer(df2,-c(FI,omega,eta2))

p1<-ggplot(df2_long,aes(x=FI,y=omega,z=R0))
p1 + geom_contour()
```
