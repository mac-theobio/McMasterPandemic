...
\begin{align}
d S_u/dt &= -\beta \frac{(I_u+I_n+I_p+I_t)}{N} S_u + \omega_{S_n} S_n \\
d S_n/dt &= f_{S_n} S_n - \omega_{S_n} S_n \\

d I_u/dt &= \beta \frac{(I_u+I_n+I_p+I_t)}{N} S_u + \omega_{I_n} I_n - f_{I_u} (1-P_{I_u}) I_u -f_{I_u} P_{I_u} I_u- \gamma I_u  \\
d I_n/dt &= f_{I_u} (1-P_{I_u}) I_u - \omega_{I_n} I_n \\
d I_p/dt &= f_{I_u} P_{I_u} I_u - \omega_{I_p} I_p \\
d I_t/dt &= \omega_{I_p} I_p - \gamma I_t  \\

d R_u/dt &= \gamma I_u + \omega_{R_n} R_n - f_{R_u} (1-P_{R_u}) R_u -f_{R_u} P_{R_u}) R_u  \\
d R_n/dt &= f_{R_u} (1-P_{R_u}) R_u - \omega_{R_n} R_n  \\
d R_p/dt &= f_{R_u} P_{R_u}) R_u - \omega_{R_p} R_p  \\
d R_t/dt &= \gamma I_t + \omega_{R_p} R_p  \\
\end{align}
