...
$B=\frac{\gamma(\omega+\gamma)(\omega+\eta_w \gamma) + \big(\gamma(\omega+\gamma)\eta_w +(\gamma \eta_w+\omega \eta_t)P_i \omega \big) F_i }{\omega+\gamma}$ and 
...
R0(params=update(params,omega=0.1))
R0(params=update(params,omega=0.2))
```
...
(2) Given that at DFE $S_n^*= \frac{t (1-P_s)}{\omega} N_0$, eq(1) will be $d S_u/dt = \frac{t N_0}{\omega}(1-P_s-\omega)$ which is always positive! So one would expect that $S_u$ grows. However we are at DFE!!!?
...
```{r contour_R0, message=FALSE,warning=FALSE }
 ...
 eta_t <- seq(0,1, length.out=11) 
 t     <- seq(0,0.1, length.out=11)
 omega <- seq(0.1,1 , length.out=11)
...
p1 <- ggplot(df2,aes(x=omega,y=Fi,z=R0))+ theme_bw() +
     theme(panel.spacing=grid::unit(0,"lines"))
(p1
    + geom_contour_filled(breaks=c(0,0.8,1,1.2,1.5,2,6), alpha=0.5) ## breaks=0.2)
    + facet_grid(eta_w~eta_t, labeller=label_both)
    ## geom_tile(alpha=0.3,aes(fill=R0),limits=c(1,3))
)
...
Interpretations:
(1) Increasing $F_i$ gives decrease of $R_0$.
(2) Increasing $\omega$ results in an increase in $R_0$.  
(3) Lowering the transmission probability for awaiting people by isolation, $\eta_w$, when $\eta_t$ is kept fixed, results in lowering the critical $F_i$. This gives a threshold for testing rate $F_i$ to achieve $R_0<1$.
(4) ?
