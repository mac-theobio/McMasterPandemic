...
**BMB**: yes, except that (1) two of the compartments are unnecessary (if we assume a perfectly specific test, i.e. no false positives, then $S_p$ and $S_t$ are always zero (2) we will want two 'integrator' or 'accumulator' compartments, $N$ and $P$, to collect cumulative reported neg/pos tests. **AGh**: Is this correct for $N$ and $P$? $dN/dt = S_n + I_n + R_n$ and $dP/dt = S_p + I_p + R_p$.
