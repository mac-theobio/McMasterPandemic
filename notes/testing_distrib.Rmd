...
$$
\frac{\int_{Q(1-t)}^1 B(y,\phi) y \, dy}{t}
$$
...
`r round(qq, 3)`, so the test positivity is `r round(qq,3)`/0.25 = r round(qq/0.25, 3)`.
...
Thus the numerator is the value we've already computed for the numerator of the proportion-positive value ($\int_{1-t}^1 x \textrm{Beta(x)}\,dx$), *minus* the product of the testing and infection means ($= ti$, using the usual identity $E[(A-\bar A)(B-\bar B)] = E[AB] - \bar A \bar B$). The variance of testing probability is that of a Bernoulli distribution ($t(1-t)$); the variance of infection probability is the variance of the Beta distribution.
...
```{r phi_corr, warning=FALSE, echo = FALSE}
...
- Why does correlation *decrease* with large $\phi$ when testing and prevalence are low ... ?? (Seems real; simulation test agrees with calculations.) (Might be obvious if we draw the pictures as above for $\phi = \{0.1, 0.375, 0.9\}$ to see what's happening.)
- ?? Is there a simple expression for cor as a function of $\phi$ (and $i$ and $t$) or *vice versa*?
