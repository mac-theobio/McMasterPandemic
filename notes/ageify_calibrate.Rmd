...
## set up simulated report data
## FIXME: FIX THIS NEXT SOMETHING IS BROKEN HERE WITH AGE-SPECIFIC REPORTS
report_data <- (res1obs
    %>% mutate(across(starts_with("report"), ~ round(.x), .names = {.col}))
    %>% select(date, starts_with("report"), var)
    %>% na.omit() ## removing first 15 values, which are NA!
)
...
<!-- Great, fitted $\beta_0$ is very close to the true value. Let's go to non-uniform contact probabilities and population distributions (one at a time, then both), and see if the fitting still works well: -->
...
<!--
Below is a calibration attempt based on following the example in macpan_base/code/breaks_calibrate_vac.R. It's not working... Abandoned in favour of using the calibrate example from the getting started vignette
-->

```{r calibrate_attempt, eval= FALSE, echo = FALSE}
## set up age categories
age_cat <- mk_agecats(0, 80, da = 30)
n <- length(age_cat)
Ni <- 1e6
...
opt_pars <- list(params=c(beta0=base_params[["beta0"]])) 
...
res_calib <- calibrate(
  ...
  , start_date = start_date
  , start_date_offset = 15 ## matches the fact that in res_base, the first 15 days have a report value od NA
	, debug_plot = FALSE
	, debug      = FALSE
	, data       = obs
	, opt_pars   = opt_pars
...
fit_params[["beta0"]] <- coef(res_calib$mle2)[[1]]

print("preset beta0:")
print(base_params[["beta0"]])
print("fitted beta0:")
print(coef(res_calib$mle2)[[1]])

res_fit <- run_sim(fit_params, base_state,
                   start_date = start_date,
                   end_date = end_date)

res_plot <- bind_rows((obs
   %>% mutate(type = "obs") 
...
p <- (ggplot(res_plot,
       aes(x = date, y = value, shape = type, colour = type))
  + geom_line()
  + geom_point()
)

(p) / (p %+% filter(res_plot, value != 0) + coord_trans(y = "log10"))

```

<!---
Fit seems sensitive to `start_date_offset` and is not as good as I would expect...
-->
