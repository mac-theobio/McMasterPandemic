...
There are currently no versioned rate matrix models. See below for experimental models.
...
* 3 components of the model that involve updates of state variables (TODO: verify that the model works for these cases at a minimum):
  * FOI
  * testing -- vec of weights that determines prob of being testing depending on state
  * vaccination -- we know realized vaccination rates, but need to scale to per-capita
* kronecker products may only be convenient for producing a matrix, rather than updating it
