...
```{r expand_state}
ss2 <- expand_stateval_age(ss, age_cat = mk_agecats(min = 1, max = 3, da = 1))
...
$$
In general, the force of infection gives the rate of disease transmission per unit time and per susceptible. A helpful way to break it down is as follows: (the average number of contacts a susceptible has with any other individual in the population per susceptible per unit time) $\times$ (the probability this contact is with an infective) $\times$ (the probability this contact between and infective and susceptible leads to transmission). 
...
In order to introduce heterogeneity in disease transmission based on age-dependent contacts, we use the basic framework of Del Valle *et. al.* (2012). 

Suppose each compartment in the model is split into $n$ different age classes. The (total) force of infection associated with susceptibles in age class $i$ can be broken down by the individual contribution of infecteds in age class $j$: $\lambda_{ij}$. We can break this force of infection down into three components, as we did in the previous section, being mindful of the fact that susceptibles are specifically of age $i$ and infecteds are of age $j$: (the average number of contacts a susceptible *of age $i$* has with any other individual in the population per susceptible *of age $i$* per unit time) $\times$ (the probability this contact is with an infective *of age $j$*) $\times$ (the probability this contact is between an infective *of age $j$* and susceptible *of age $i$* leads to transmission). Since we want to assume that the average number of contacts between age groups varies based on the ages involved, the first two terms need to be modified as follows: (the average number of contacts a susceptible of age $i$ has *with an individual of age $j$* per susceptible *of age $i$* per unit time) $\times$ (the probability this contact *with an individual of age $j$* is with an infective).

Translating the above into notation, we have

$$
\lambda_{ij} = c_{ij} \times \frac{I_j}{N_j} \times p_{ij},
$$

where $c_{ij}$ is the average number of contacts a susceptible of age $i$ has with an individual of age $j$ per susceptible of age $i$ per unit time, $I_j/N_j$ is the probability this contact with an individual of age $j$ is with an infective, and $p_{ij}$ is the probability this contact between an infective of age $j$ and susceptible of age $i$ leads to transmission.

We still need to determine how we will model $c_{ij}$ and $p_{ij}$. 

In the Del Valle paper, they choose a Poisson model for $p_{ij}$ using the age-dependent mean duration of the contact $t_ij$ and a mean number of transmission events per unit time $\xi$: $p_{ij} = 1- e^{\xi t_{ij}}$. It's probably fine to just take $p_{ij} = p$ as constant across age classes for now. (One could also incorporate susceptibility of age $i$ individuals and infectivity of age $j$ individuals, as Del Valle does, but I'm omitting that here for simplicity.)

For $c_{ij}$, the Del Valle paper calculates this quantity using a matrix of *total* contacts between ages $i$ and $j$ per unit time $\{C_{ij} \}$, and then row normalizes by the age-specific population ($N_i$) to get the average number of contacts between ages $i$ and $j$ per individual of age $i$ per unit time. While the matrix $\{C_{ij} \}$ is necessarily symmetric, $\{c_{ij} \} = \{C_{ij}/N_i \}$ is not, unless the population is uniformly distributed across ages.

So, we can express the age-specific force of infection as
$$
\lambda_{ij} = \frac{C_{ij}}{N_i} \times \frac{I_j}{N_j} \times p,
$$

where $C_{ij}$ is the total number of contacts between ages $i$ and $j$ per unit time, $N_i$ and $N_j$ are the age-specific population sizes of ages $i$ and $j$, respectively, and $p$ is the constant probability of infection given contact between a susceptible and infective. 

This would imply that $\beta_{ij} = (C_{ij}/N_i)p$ is the age-specific transmission rate. If we assume $C_{ij}$ from studies of contact frequency and $N_i$ from census data, the only thing left to callibrate is $p$.

# Existing contact data

## Prem *et al.*

The extrapolate from POLYMOD + some demographic surveys using a Baysian hierarchical model to generate matrices for countries that weren't in the original Polymod set. They also break up the contact matrix into components based on setting. 

## Mistry *et al.*

These synthetic matrices constructed from census and survey data and are readily available on [Github](https://github.com/mobs-lab/mixing-patterns). They offer four setting-specific contact matrices with "the per capita *probability* of contact for *an* individual of age i with individuals of age j in that setting k". They also offer an aggregate contact matrix using a linear combo of the four setting-specific ones; they weights they use in this linear combo are "the average number of effective contacts made in the setting that can lead to disease transmission". These quantities are disease specific, but they fit under the conditions for an airborne disease to spread across several countries from "diary-based" survey contact matrices. So the overall matrix gives "per capita number of contacts an individual of age i has with individuals of age j".

There is a specific matrix for Ontario (and the other provinces/territories), so that's appealing. The age groups are predetermined as 1-year ages to 84, then 85+, but they also give the age distributions they use (so we can verify if those are sensisible). If the normalization is as I expect it is, we could recover the non-normalized versions of the matrices and normalize with our own population distributions from StatCan.

Let's look at one example of a setting-specific contact matrix:

```{r mistry-contacts-ON}
## this is the overall contact rate linear comb of the setting-specific frequency matrices, 
...
This paper discusses the application of their matrices to an age-structured SIR model, and at first glance it's pretty similar to the Del Valle framework. Need to look at how they define $\beta$ to make sense of it. They say "$\beta$ is the transmissibility of the infection", but nothing else... no units, no bounds. But it is constant across ages (of both S and I). In the Fig. 7 caption, they say "The transmission rate is set such that R0 = 1.5 when using the country-level matrix", so they fit $\beta$ based on the desired $\mathcal{R}_0$, all else being equal.

# Remaining questions

* How do we connect this back to $\beta$(s) calibrated from data in MacPan?
