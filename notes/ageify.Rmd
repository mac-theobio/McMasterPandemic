...
## default... is there a good reason not to convert them to
## (named) lists by default so that we can provide
## age-structured parameters as a named vec in a list entry
## e.g. for delta = frac of acute care cases that are fatal
## pp$delta = P(death|hosp) from age struct paper
## names(pp$delta) = attr(pp, "age_cat")
## --- will this mess with calibration? would need to check
## unit tests

## FIXME: have read_params check for age structure
## (maybe grep on "\_[:digit:]+-[:digit:]+) and if so,
## attach an "age_cat" attribute with the list of age groups
## found to the params_pansim object 

## TODO: verify input setup with BB. will the implied
## param.csv structure work well (e.g. having
## MODELCAT_AGECAT as param names)?
...
## TODO: ask BB about what the get_evec stuff does in
## make_state with the default method = expsim... looks like
## we start with a state where N = 1, E0 = 1e-5, run an
## exponential simulation with the given params for 100
## steps, takes the state at the last time step (minus
## non-infected states) and normalizes it using the sum,
## returning this as the eigenvector... why? what's the math
## behind this?
...
## TODO: ask BB about why we want an infective when we have
## exposed individuals in ss2 already
```
...
(*IP:*: what are the units on the `beta` "vector"/matrix? `beta0` in the param file? What are the implied units on `Cmat`? Should add units for these quantities (and others) in the documentation/included in param descriptions for parsets that ship with the package.)
...
(*IP:* This function runs `run_sim_range` if the simulation is deterministic and with constant parameters.) 
