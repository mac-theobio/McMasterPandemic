...
unif is practically identical,
but diag is quite different: do we actually expect to be the same? under what conditions/scaling?

```{r}
mm <- mdiff_fun(sims[c("non_age","diag","unif")])
gg4 <- ggplot(mm,aes(date,diff,colour=lab)) + geom_line()+facet_wrap(~var,scale="free") + scale_y_log10()
print(gg4)
```

## checking diagonal case

Focus on the first few steps ...
	
```{r}
M.u <- make_ratemat(ss2, ppa.u, sparse=TRUE)
M.d <- make_ratemat(ss2, ppa.d, sparse=TRUE)
nz(condense_age(do_step(ss2, ppa.u, M.u)))
nz(condense_age(do_step(ss2, ppa.d, M.d)))
mmh <- mm %>% head(120) %>% filter(var %in% c("S","E","I","H"))
gg4 %+% mmh + geom_point()
```
