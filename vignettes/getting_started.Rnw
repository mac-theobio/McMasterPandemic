...
<<make.long.form.reports.and.deaths, message=FALSE>>=
library(tidyr)
...
<<calibrateTOres1obs.both.reports.and.deaths, message=FALSE>>=
## beta0 is the only parameter we're going to optimize:
opt_pars <- list(params = c(beta0=0.1))
fitted.mod <- calibrate(
...
<<troubleshooting.ndt, eval=FALSE>>=
    , sim_args = list(ndt = 2)
@ 
\noindent
...
<<time.varying.bt.using.run_sim, message=FALSE>>=
library(zoo)
library(tidyverse)
@ 
\noindent
