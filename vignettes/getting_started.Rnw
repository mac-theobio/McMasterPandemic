...
Imagine that the simulated data saved in \code{res1proc2} are were the
...
@ 
Note that the when we print the fitted parameters values, we get 
a list with two elements (\code{params} and \code{nb_disp})

Try now with observation noise only, since we have no way to fit
process noise in our fitting machinery.
...
Now try to fit to both reports and deaths.

<<calibrateTOres1obs.both.reports.and.deaths>>=
report_data <- (res1obs 
