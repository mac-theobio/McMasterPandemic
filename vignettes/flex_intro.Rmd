...
```{r}
sir_model$ratemat = as.matrix(sir_model$ratemat)
(sir_model
    ...
    %>% ggplot
     +  geom_line(aes(x = time, y = state, colour = epi_cat))
  )
```

```{r}
(sir_model
    %>% simulate_changing_ratemat_elements
    %>% mutate(time = row_number())
    %>% rename(`force of infection` = S_to_I)
    %>% ggplot
     +  geom_line(aes(x = time, y = `force of infection`))
  )
```
...
```{r}
two_strain_model$ratemat = as.matrix(two_strain_model$ratemat)
regex = "^(S|I|R)(_*)(.*)"
(two_strain_model
    %>% simulate_state_vector
    %>% mutate(time = row_number())
    ...
    %>% ggplot
     +  geom_line(aes(x = time, y = state, colour = epi_cat, linetype = strain))
  )
```
...
(two_strain_model
    ...
    %>% rename(strain = name)
    %>% ggplot
     +  geom_line(aes(x = time, y = `force of infection`, colour = strain))
  )
```
