...
TODO: make handling of transformations consistent. the regularization functions need to be computed on the transformed scale, but the epidemiological simulations need to be treated on the original scale. therefore, it is probably best to compute the priors as soon as the parameters get read in by the `PARAMETER_*` macros, then compute the inverse transformations so that the parameters are on the right scale for the simulations. this way we don't have to undo the inverse transformations just to compute the regularization functions after the simulations.
