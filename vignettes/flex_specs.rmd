...
2. After the simulation loop create a matrix (call this matrix `sim_report`) with rows that correspond to time steps and columns that correspond to each variable stored in the following.
  - `concatenated_state_vector`
  - `concatenated_ratemat_nonzeros`
  - `concatenated_sums`
  - `concatenated_factrs`

3. Loop over the rows (i.e. time steps) of `sim_report` and at each iteration do the following three sets of operations and add the results as columns in `sim_report`:
  - a) compute the same set of user-defined expressions containing sums and products of the columns in `sim_report`
  - b) compute lag-n operations of a user-defined set of the columns of `sim_report`
  - c) compute convolutions of a user-defined set of the columns of `sim_report`
...
* Sum/product expressions
  - `condense_count` -- vector the same length as the number of columns added in step 3a giving the number of `sim_report` columns to use as input for each new column (note that input columns can be used more than once -- see `count` vector)
  - `condense_sri` -- indices into the columns of `sim_report` to use as input (see `spi` vector)
  - `condense_modifier` -- TODO (similar to `modifier`)
* Lag-n operations
  - `lag_sri` -- indices into the columns of `sim_report` to use as input to lag-n operations
  - `lag_order` -- vector same length as `lag_spi` containing the order (i.e. the "n" in lag-n) of the lag for each lag-n operation
* Convolutions
  - `conv_spi` -- indices into the columns of `sim_report` to use as input to convolution operations
...
Add `sim_report` to the report. On the R-side we could start using `sim_report` where we currently use `concatenated_*` vectors, and eventually we could drop them.
