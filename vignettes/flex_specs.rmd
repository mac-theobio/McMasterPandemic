...
-   [0.2.1 -- Log-linear exogenous time-variation of
    parameters](#v0.2.1)
...
where $q$ is a constant integer vector that gets passed in through the `DATA` macro.

Note that in classic MacPan `q` is computed as follows. 
```{r eval = FALSE}
1:ceiling(qgamma(0.95, shape, scale))
```
But we are avoiding this because it will introduce a discontinuity that could cause automatic differentiation to fail. This difference between refactored and classic MacPan only has the potential to cause numerical differences whenever `params['c_delay_cv']` and `params['c_delay_mean']` are varying -- typically because they are in `opt_pars` -- but I have not yet seen any examples where this is the case. To guarantee matched results at least for the initial parameter vector one should set `q` as follows on the R-side.
```{r eval = FALSE}
q = 1:ceiling(qgamma(0.95, 1/params[["c_delay_cv"]]^2, params[["c_delay_mean"]] * params[["c_delay_cv"]]^2))`
```
...
TODO: in collaboration with Weiguang

Start with notes on what is done and what is not done
...
Also note that items 4 and 5 could be computed together using machinery that could be modified from the `update_factr_in_sp` machinery (note further that the `update_factr_in_sp` machinery has already been modified from the `make_ratemat` machinery).

Development sequence that would lead us to a solution:

1. Create two new concatenated vectors that store the history of the simulations and put these in the report:
  - `concatenated_sums` -- these are the items in `sp` corresponding to sums of states and parameters (see `update_sum_in_sp`)
  - `concatenated_factrs` -- these are the items in `sp` corresponding to so called factrs (see `update_factr_in_sp`)

2. After the simulation loop create a matrix (or appropriately indexed vector if that is more efficient) with rows that correspond to time steps and columns that correspond to each variable stored in `concatenated_state_vector`, `concatenated_ratemat_nonzeros`, `concatenated_sums`, `concatenated_factrs` -- call this matrix (or vector) `sp_condense`

3. Loop over the rows (i.e. time steps) of `sp_condense` and at each iteration do the following three sets of operations and add the results as columns in `sp_condense`:
  - a) compute the same set of user-defined expressions containing sums and products of the columns in `sp_condense`
  - b) compute lag-n operations of a user-defined set of the columns of `sp_condense`
  - c) compute convolutions of a user-defined set of the columns of `sp_condense`

Note that development steps 1-2 require no new user input, although step 3 requires the following input.

* Sum/product expressions
  - `count_condense` -- vector the same length as the number of columns added in step 3a giving the number of `sp_condense` columns to use as input for each new column (note that input columns can be used more than once -- see `count` vector)
  - `spi_condense` -- indices into the columns of `sp_condense` to use as input (see `spi` vector)
  - `modifier_condense` -- TODO (similar to `modifier`)
* Lag-n operations
  - `lag_spi` -- indices into the columns of `sp_condense` to use as input to lag-n operations
  - `lag_order` -- vector same length as `lag_spi` containing the order (i.e. the "n" in lag-n) of the lag for each lag-n operation
* Convolutions
  - `conv_spi` -- indices into the columns of `sp_condense` to use as input to convolution operations
  - `conv_c_prop_i` -- vector same length as `conv_spi` giving indices into `params` for extracting the parameters of the convolutions

Add `sp_condense` to the report. On the R-side we could start using `sp_condense` where we currently use `concatenated_*` vectors, and eventually we could drop them.


##### First Attempt

At a high level condensation is comprised of a transformation of `concatenated_state_vector` into `condensed_state_vector`, which is typically smaller. The condensation algorithm has seven phases, corresponding to the seven types of variables that can be added to `condensed_state_vector`.
...
1. Let all elements of the state vector for the full model, $s$, and linearized model, $\hat{s}$, be zero: $s_i = \hat{s}_i = 0$
2. Make two copies of the parameter vector -- one for the full model, $\theta$, and linearized model, $\hat{\theta}$
3. Update a set of the parameters in $\hat{\theta}$ with values that will remain constant even if the parameter vector $\theta$ is updated
4. Update a set of the state variables in $\hat{s}$ with a set of parameter values in $\hat{\theta}$
5. Create a function of the state vector 
5. Compute the Jacobian matrix of a function of the state vector -- using automatic differentiation --   with respect to the state vector  on a function that updates $\hat{s}$ with the following steps
