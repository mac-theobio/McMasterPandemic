...
time_pars <- data.frame(Date=c("2020-03-01"),
                        ...
                        Adjustment_type = c("absolute"),
                        Adjustment_value=c(1e5))
...
p2 <- (ggplot(get_doses_per_day(res_vax),
       ...
       + geom_point()
       + geom_vline(aes(xintercept = as.Date(time_pars$Date)),
                    linetype = "dashed",
                    colour = "grey60")
       + annotate("text",
                  x = as.Date(time_pars$Date)-8,
                  y = 0.5*time_pars$Adjustment_value,
                  angle = 90,
                  colour = "grey60",
                  label = paste0("vaccination starts\nat rate of\n",
                                 format(time_pars$Adjustment_value, scientific = FALSE, big.mark = ","), " doses/day"))
)
