...
test_that("non-integer values throw error", {
    expect_error(calibrate(data=dd_nonint,
                           base_params=params,
                           opt_pars=opt_pars0,
                           time_args=NULL),
                 "need integer values")
...
test_that("calibrate parameters other than beta0", {
  ...
  time_pars <- data.frame(Date=c("2020-05-01","2020-07-01"),
                          Symbol=c("phi2","beta0"),
                          Relative_value=c(1e-3,1e-3))
  sd <- "2020-03-01"; ed <- "2020-10-02"
  SS <- run_sim(params,
                state,
                start_date = sd,
                end_date = ed,
  ...
  ggplot(dd,aes(date,value,colour=var)) + geom_line() + scale_y_log10()
  if (FALSE) {
