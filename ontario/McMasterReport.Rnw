...
p <- (ggplot(ont_noICU, aes(x=date, y=value, colour=var))
    ...
    + geom_line(alpha=1/3)
    + geom_point()
    + ggtitle("Data used in this report")
...
\caption{The daily Ontario COVID-19 data that we use to calibrate our
  ...
  dotted lines indicate the dates on which successive social
  distancing policies were implemented.
  \david{I'd like to show the mobility data we are using on this graph
  as well.}
}
  \label{fig:data}
\end{figure}
