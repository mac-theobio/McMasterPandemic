...
\begin{itemize}
\item \david{Date of latest data update should be noted here.  This
    could be extracted by finding the last observation time in the
    relevant \texttt{RData} file.}
\item Social distancing measures have reduced the effective
  ...
  with 95\% confidence).  \david{The fits are quite unstable.  With
    just 3 more days of data, we predict that we have already peaked.}
...
best fit changes.  \david{We should state what interventions were
  implement on each of these dates.}
...
<<ont_calibration_plot, echo=FALSE, warning=FALSE, fig.height=8>>=
## see ?plot.fit_pansim
ont_fit <- ont_cal1 # ont_cal1 is built in to pkg
...
\begin{figure}
<<ont_forecast_plot, echo=FALSE, warning=FALSE, fig.height=8>>=
## extend fit to August
ont_fit_forecast <- ont_fit
attr(ont_fit_forecast, "forecast_args")$end_date <- "2020-10-31"
plot(ont_fit_forecast
     , data = ont_trans
     , add_tests = TRUE
     , add_ICU_cap = TRUE
     , directlabels = TRUE
