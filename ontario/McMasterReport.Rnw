...
\item Social distancing measures have reduced the effective
  ...
  with 95\% confidence).  \david{The fits are quite unstable with all
    ...
    we have already peaked.  Ignoring the first break point (what
    we're doing now), we get a very decent-looking fit.  Note that DC
    uses only one break point at the start of social distancing.}
...
\item \david{We are currently assuming a 5 day lag between infection
    and report.  In contrast, DC (following advice from Kevin Brown)
