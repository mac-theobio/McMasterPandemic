...
\item Social distancing measures have reduced the effective
  ...
  with 95\% confidence).  \david{The fits are quite unstable with all
    three break points: with just 3 more days of data, we predict that
    we have already peaked.  Ignoring the first break point, we get a
    very decent-looking fit (which is what we're doing at the
    moment).}
