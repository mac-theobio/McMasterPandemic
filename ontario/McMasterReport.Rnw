...
\item Social distancing measures have reduced the effective
  reproduction number ($\R_t$) substantially, but not to the point of
  suppression (we still estimate $\R_t>1$ with 95\% confidence).
...
\item ICU counts continue to diverge fairly sharply from the model
  ...
  counts are still well below capacity.
  \begin{itemize}
...
\item Testing lags for HCWs are likely much shorter than the general
  public.
\item A large proportion of deaths are residents of long term care
  facilities (LTCFs), which are not modelled explicitly.
\item At present, we do not attempt to model any heterogeneities of
  transmission associated with age.
\item We now have access to PHO's non-public data, but have not yet
  incorporated it.
\end{itemize}
...
\item Compartmental mechanistic model:
  ...
  are presumed to have had a period of presymptomatic infectiousness.
...
load("ontario_calibration.RData")
param_table <- describe_params(summary(params))
beautify <- function(x) {x[,"symbol"] <- texify(as.character(x[,"symbol"]), force=TRUE); return(x)}
param_table <- beautify(param_table)
estimated_params <- invlink_trans(restore(ont_cal1$par,opt_pars))
main_estparam_table <- beautify(describe_params(estimated_params$params))
##Rt_params <- 
@ 
...
<<summary_parameter_table, echo=FALSE, results="asis">>=
...
rel_Rt_table <- data.frame( bd = bd, 
                            rel_beta0 = estimated_params$rel_beta0 )
rel_Rt_table[,"rel_beta0"] <- round(rel_Rt_table[,"rel_beta0"], 2)
colnames(rel_Rt_table) <- c("Intervention Date","Change in ${\\mathcal R}_t$")
@ 

Table~\ref{Rttab} shows estimated relative changes in $\R_t$ on the
dates on which social distancing interventions were implemented.  We
assumed $\R_t$ changes abruptly on each of these dates and found the
best fit change.
<<Rt_table, echo=FALSE, results="asis">>=
Hmisc::latex(rel_Rt_table, file="", label="Rttab", rowlabel="", table.env=TRUE
...
\david{I guess this is based on fitting to ICU? or heavily weighting ICU?}

The vertical lines show the times of implementation of different social distancing measures
(\emph{cf.}~Table~\ref{Rttab}).
...
\begin{figure}
  ...
  \caption{COVID-19 forecasts.
    Dots show reported data.  Curves show forecasts.
...
error in very light colours under the deterministic curves.  Some kind
