...
load("ontario_calibration_noICU_2brks_prior.RData")
...
\item Social distancing measures have reduced the effective
  ...
  with 95\% confidence).  \ben{The fits are \textbf{still} quite
    unstable, even with only two break ponts \ldots} \david{Would it
    be reasonable to add: ``It is important to note that the fits are
    unstable, in the sense that relatively small changes in unknown
    parameter values (e.g., mean time from infection to symptom onset)
    can yield large changes in peak time and peak height.''}
...
Using hospitalization-only fit (2 breaks, no priors) right now.
...
\begin{figure}
<<ont_forecast_plot, echo=FALSE, warning=FALSE, cache=TRUE, fig.height=9>>=
## extend fit to August
pp <- predict(fit
            , stoch=c(obs=TRUE,proc=TRUE)
            , ensemble=TRUE
