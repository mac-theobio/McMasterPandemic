...
NS_splines_ode <- do.call(calibrate_comb
	, c(nlist(params=params
		, debug_plot=FALSE
...
BS_splines_ode <- do.call(calibrate_comb
	, c(nlist(params=params
		, debug_plot=FALSE
		, sim_args = list(use_ode = TRUE)
		, mle2_args = list(skip.hessian=TRUE)
...
print(plot(NS_splines_ode, data=calibrate_data_fill) + ggtitle("NS splines ode"))

print(plot(BS_splines_ode, data=calibrate_data_fill) + ggtitle("BS splines ode"))
