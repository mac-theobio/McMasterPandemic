...
```{r ont_calibrate 1stwave, warning=FALSE}
...
ggmob <- (ggplot(clean_mobdat,aes(date,rel_activity))
	+ geom_point()			 
)
...
```{r ont_calibrate_mob_breaks, warning=FALSE}
...
ontmod_mob2 <- calibrate_comb(data = calibrate_dat
	...
	, mob_data = clean_mobdat
	, use_mobility = TRUE
	, mob_breaks = c("2020-04-15", "2020-08-8")
	, mob_breaks_int = TRUE
	, mob_logist_scale = 3
)
